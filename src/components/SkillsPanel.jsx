      ...
      {groups.map((group) => {
        ...
        return (
          <article className="border-b border-border" key={group.label}>
            <button
              type="button"
              className={`flex w-[calc(100%+24px)] -ml-3 items-center justify-between gap-5 rounded-lg border-0 px-3 py-[18px] text-left text-ink transition duration-150 hover:bg-paper-soft focus-visible:bg-paper-soft ${open ? 'bg-paper-soft' : 'bg-transparent'}`}
              aria-expanded={open}
            ...
            >
              <span className="grid gap-[5px]">
                ...
                <span className="text-xs text-muted">{open ? 'Hide skills' : 'View skills'}</span>
              </span>
            ...
            <div className={`grid grid-rows-[0fr] transition-[grid-template-rows] duration-200 ${open ? 'grid-rows-[1fr]' : ''}`} id={panelId} aria-hidden={!open}>
              <div className="min-h-0 overflow-hidden">
                <div className="px-[18px] pt-2.5 pb-[26px]">
